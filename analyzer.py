        ...
        for i, pkt in enumerate(cap):
            ...
            if hasattr(pkt, "dns"):
                ...
                if len(seen_dns_queries) < 5:
                    print(
                        f"[DEBUG] DNS packet {i+1} attributes: {list(pkt.dns._all_fields.keys())}"
                    )
                ...
                if (
                    hasattr(pkt.dns, "flags_tree")
                    and pkt.dns.flags_tree.get("dns.flags.response") == "1"
                ):
                        ...
                        if isinstance(answers, dict):
                            for answer_key, answer_data in answers.items():
                                if isinstance(answer_data, dict):
                                    ...
                                    if ":" in answer_key:
                                        response_domain = answer_key.split(":")[
                                            0
                                        ].strip()
                    ...
                    if resolved_ip and response_domain:
                        ...
                        if response_pair not in seen_dns_responses:
                            ...
                            print(
                                f"{'RESPONSE':<10} | {response_domain:<50} → {resolved_ip}"
                            )
                    ...
                    # Debug: Show response packet structure for first few
                        ...
                        if "Answers" in pkt.dns._all_fields:
                            ...
                            print(
                                f"[DEBUG] No Answers field, available: {list(pkt.dns._all_fields.keys())}"
                            )
